...
from logoServices import get_logo_by_text
from logoServices import get_logo_by_image

webp = jp.WebPage()
INPUT_IMAGE_DIR = "/static/logo_input/"

async def search_handler(self, msg):
	self.div.text = self.value

	# execute vector search in Astra DB
	response = await get_logo_by_text(self.value)
...
async def file_input_handler(self, msg):
	...
	# execute vector search in Astra DB
	response = await get_logo_by_image(f.name)
...
# top banner
jp.Img(src="/static/web_images/logo.png", a=webp)
